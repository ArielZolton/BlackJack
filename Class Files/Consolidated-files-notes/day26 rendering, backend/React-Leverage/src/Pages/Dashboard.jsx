...
import { Typography, CircularProgress, Grid, Stack, Skeleton } from "@mui/material";
...
export default function Dashboard() {
  ...
  return (
    <div>
      ...
      {/* <Typography variant="h4">No users found</Typography>         instead of div^^ */}
        ...
        {status === "loading" &&
          Array.from({ length: 3 }).map((_, index) => {
            return (
              <Skeleton
                key={index}
                variant="rectangular" // or rounded???
                height={220}
                sx={{ bgcolor: "grey.200" }}
              />
            );
